...
<img src="assets/agent-selector-hero.png" alt="agent-selector — Vedic Arsenal by Darshankumar Joshi" width="100%" />
...
```mermaid
flowchart LR
    A[Input] --> B[agent-selector]
    B --> C{Process}
    C -- Success --> D[Output]
    C -- Error --> E[Handle / Retry]
    E --> B
    style B fill:#6b21a8,color:#fff
    note["Selector — Zero Dependencies"]
```
